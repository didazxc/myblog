<template>
  <div>
    <div
      v-if="isDrawer && !collapse"
      class="modal"
      @click="handleCollapse"
    ></div>
    <aside :class="{ drawer: isDrawer, close: isDrawer && collapse }">
      <div class="aside-header" @click="handleCollapse">
        <img :src="`${publicPath}favicon.ico`" alt="C" />
        <span v-if="!(!isDrawer && this.collapse)"><b>{{title}}</b></span>
      </div>
      <el-menu
        router
      ...
      >
        <template v-for="(v, k) in menus">
          <el-menu-item v-if="v.index !== undefined" :key="k" :index="k">
